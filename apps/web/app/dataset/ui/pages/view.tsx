        ...
        <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
          <aside className="rounded-2xl border bg-card p-4 shadow-sm">
            <p className="text-sm font-medium text-foreground">
              {t('dataset.view.page.datasets')}
            </p>

            {datasets.length === 0 ? (
              <p className="mt-4 text-sm text-muted-foreground">
                {t('dataset.view.page.empty')}
              </p>
            ) : (
              <div className="mt-4 space-y-2">
                {datasets.map((dataset) => (
                  <Link
                    ...
                    className={`block rounded-md border px-3 py-2 text-sm transition ${dataset.id === selectedDatasetId
                ...
                <div className="mt-2 space-y-2">
                  {selectedDataset.versions.map((version) => (
                    <Link
                      ...
                      className={`block rounded-md border px-3 py-2 text-xs transition ${version.id === selectedVersionId
                        ? 'border-primary bg-primary/5 text-foreground'
                        : 'text-muted-foreground hover:text-foreground'
                        }`}
                    >
          ...
          <section className="rounded-2xl border bg-card p-6 shadow-sm">
                    ...
                    <Field name="description">
                      <label className="grid gap-2">
                        ...
                        <textarea
                          ...
                          rows={3}
                          placeholder={t(
                            'dataset.view.page.update.description.placeholder'
                          )}
            ...
            <div className="mt-6">
              <h4 className="text-base font-semibold">
                {t('dataset.view.page.readme_title')}
              </h4>
